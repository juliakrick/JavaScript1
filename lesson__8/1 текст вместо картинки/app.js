...
function handleClick(clickedButtonEvent) {
 ...
 let textBtn = clickedButtonEvent.target.textContent
    ...
    if (textBtn == 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if(textBtn == 'Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function hideMoreText(card) {
    ...
    let descEl = document.querySelector('.desc').remove()
    // descEl.remove
    // card.wrap.remove()
